...
local WeatherForecastEntity = {}
WeatherForecastEntity.__index = WeatherForecastEntity


function WeatherForecastEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WeatherForecastEntity)
  self._name = "weather_forecast"
  self._client = client
...
function WeatherForecastEntity:get_name()
  return self._name
end


function WeatherForecastEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WeatherForecastEntity.new(self._client, opts)
end
...
function WeatherForecastEntity:mark_deleted()
  self._deleted = true
end


function WeatherForecastEntity:deleted()
  return true == self._deleted
end


function WeatherForecastEntity:data_set(args)
  if args ~= nil then
...
function WeatherForecastEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WeatherForecastEntity:match_set(args)
  if args ~= nil then
...
function WeatherForecastEntity:match_get()
...
function WeatherForecastEntity:stream(action, args, callopts)
...
function WeatherForecastEntity:_run_op(ctx, post_done)
  local utility = self._utility
