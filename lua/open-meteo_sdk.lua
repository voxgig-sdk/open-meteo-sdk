...
local OpenMeteoSDK = {}
OpenMeteoSDK.__index = OpenMeteoSDK
...
OpenMeteoSDK._make_feature = _make_feature


function OpenMeteoSDK.new(options)
  local self = setmetatable({}, OpenMeteoSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function OpenMeteoSDK:options_map()
...
function OpenMeteoSDK:get_utility()
  return Utility.copy(self._utility)
end


function OpenMeteoSDK:get_root_ctx()
  return self._rootctx
end


function OpenMeteoSDK:prepare(fetchargs)
...
function OpenMeteoSDK:direct(fetchargs)
...
-- by passing it.
function OpenMeteoSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function OpenMeteoSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "OpenMeteoSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function OpenMeteoSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = OpenMeteoSDK.new(sdkopts)
  sdk.mode = "test"
