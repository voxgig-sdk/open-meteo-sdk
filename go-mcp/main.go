// Command OpenMeteo-mcp is an MCP server exposing the OpenMeteo
// SDK as MCP tools.
//
// Two tools are registered:
//
//	OpenMeteo_list  — list records of an entity
//	OpenMeteo_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: OPEN_METEO_APIKEY carries the API key and
	// OPEN_METEO_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPEN_METEO_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPEN_METEO_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenMeteoSDK(opts)
