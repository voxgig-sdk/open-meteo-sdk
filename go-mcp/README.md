...
[MCP](https://modelcontextprotocol.io) server exposing the OpenMeteo SDK as
two agent tools — `open-meteo_list` and `open-meteo_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-meteo-mcp)
make build

# 2. Provide credentials via the environment
export OPEN_METEO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user open-meteo \
  -- /absolute/path/to/open-meteo-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./open-meteo-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// open-meteo_list: first page of records
{ "entity": "historical" }
{ "entity": "historical", "query": { } }

// open-meteo_load: one record by id
{ "entity": "historical", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-meteo-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user open-meteo \
     -- "$PWD"/dist/*/open-meteo-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `open-meteo_list` and `open-meteo_load` tools now appear
   in new sessions. Ask the agent to *"list historical using open-meteo"*
   and it calls `open-meteo_list` with `{"entity":"historical"}`.
...
```sh
export OPEN_METEO_APIKEY=sk_live_xxx            # API key
export OPEN_METEO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `open-meteo_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `open-meteo_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPEN_METEO_APIKEY` | API key sent with every request. |
| `OPEN_METEO_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

historical | marine_forecast | weather_forecast

### Smoke test via HTTP (raw JSON-RPC)

```sh
./open-meteo-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"open-meteo_load","arguments":{"entity":"historical","query":{"id":1}}}}'
```
