...
class OpenMeteoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            OpenMeteoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = OpenMeteoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = OpenMeteoTestRunner.getenv("OPEN_METEO_TEST_LIVE")
        override = OpenMeteoTestRunner.getenv("OPEN_METEO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = OpenMeteoTestRunner.getenv(key)
        ...
        explain = OpenMeteoTestRunner.getenv("OPEN_METEO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["OPEN_METEO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if OpenMeteoTestRunner._test_control is not None:
            return OpenMeteoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                OpenMeteoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            OpenMeteoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = OpenMeteoTestRunner.load_test_control()
...
def load_env_local():
    OpenMeteoTestRunner.load_env_local()


def env_override(m):
    return OpenMeteoTestRunner.env_override(m)


def entity_data(v):
    return OpenMeteoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return OpenMeteoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return OpenMeteoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return OpenMeteoTestRunner.load_test_control()


def live_client_options():
    return OpenMeteoTestRunner.live_client_options()


def live_delay_ms():
    return OpenMeteoTestRunner.live_delay_ms()
