...
class TestWeatherForecastDirect:

    def test_should_direct_load_weather_forecast(self):
        setup = _weather_forecast_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-weather_forecast", "live" if setup["live"] else "unit")
        if _skip:
...
def _weather_forecast_direct_setup(mockres):
