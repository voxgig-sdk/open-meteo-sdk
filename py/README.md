...
The Python SDK for the OpenMeteo API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Historical(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.OpenMeteo(None).load(
    {"id": "test01"}, None
)
