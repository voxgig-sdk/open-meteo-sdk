...
//     (\`new OpenMeteoSDK(...)\` / \`OpenMeteoSDK.test(...)\` become
//     \`OpenMeteoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"historical":{"test01":{"id":"test01"}},"marine_forecast":{"test01":{"id":"test01"}},"weather_forecast":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new OpenMeteoSDK(...)\` and any
// \`OpenMeteoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+OpenMeteoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/OpenMeteoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
