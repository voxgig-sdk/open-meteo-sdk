...
```ts
const result = await client.Historical().load({ end_date: 'end_date', latitude: 1, longitude: 1, start_date: 'start_date' })
```
...
```ts
const result = await client.MarineForecast().load({ latitude: 1, longitude: 1 })
```
...
```ts
const result = await client.WeatherForecast().load({ latitude: 1, longitude: 1 })
```
