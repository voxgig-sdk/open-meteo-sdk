...
import (
	...
	omni "github.com/voxgig-sdk/open-meteo-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.OpenMeteoError); is {
				out := map[string]any{
					"name":    "OpenMeteoError",
...
func omniCtx(arg any, client *sdk.OpenMeteoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.OpenMeteoSDK); is {
			client = live
