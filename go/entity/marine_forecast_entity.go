...
func (e *MarineForecastEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *MarineForecastEntity) Deleted() bool {
	return e.deleted
}
...
// argument it returns the current data as an MarineForecast; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *MarineForecastEntity) DataTyped(data ...MarineForecast) MarineForecast {
	if len(data) > 0 {
		return typedFrom[MarineForecast](e.Data(asMap(data[0])))
	}
	return typedFrom[MarineForecast](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through MarineForecast (all fields
// optional at the wire level).
func (e *MarineForecastEntity) MatchTyped(match ...MarineForecast) MarineForecast {
	if len(match) > 0 {
		return typedFrom[MarineForecast](e.Match(asMap(match[0])))
	}
	return typedFrom[MarineForecast](e.Match())
}
...
func (e *MarineForecastEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
// MarineForecastLoadMatch and returns an MarineForecast. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *MarineForecastEntity) LoadTyped(reqmatch MarineForecastLoadMatch, ctrl map[string]any) (MarineForecast, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return MarineForecast{}, err
	}
	return typedFrom[MarineForecast](res), nil
}
