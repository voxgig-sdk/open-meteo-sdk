...
func (sdk *OpenMeteoSDK) OptionsMap() map[string]any {
...
func (sdk *OpenMeteoSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *OpenMeteoSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *OpenMeteoSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OpenMeteoSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *OpenMeteoSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *OpenMeteoSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath(sdk.options, []any{"allow", "op"}).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("OpenMeteoSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *OpenMeteoSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *OpenMeteoSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("OpenMeteoSDK: graphql: %s", msg)
		res["graphql"] = errors
	}
