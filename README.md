...
Learn more about Open Meteo at [open-meteo.com](https://open-meteo.com).

This is an unofficial SDK for the Open Meteo public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Historical, MarineForecast and WeatherForecast — that you
...
const historical = await client.Historical().load({ end_date: "example", latitude: 1, longitude: 1, start_date: "example" })
```
...
| --- | --- | --- |
| **Historical** | The Historical entity (load). | `/v1/historical` |
| **MarineForecast** | The MarineForecast entity (load). | `/v1/marine-weather` |
| **WeatherForecast** | The WeatherForecast entity (load). | `/v1/forecast` |
