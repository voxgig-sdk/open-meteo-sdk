...
class OpenMeteoSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = OpenMeteoHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = OpenMeteoHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!OpenMeteoFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, OpenMeteoFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return OpenMeteoUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = OpenMeteoHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = OpenMeteoHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = OpenMeteoHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new OpenMeteoError($op . "_allow",
                "OpenMeteoSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = OpenMeteoHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new OpenMeteoError("graphql_error",
                "OpenMeteoSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function MarineForecast($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_marine_forecast === null) {
                $this->_marine_forecast = new MarineForecastEntity($this, null);
            }
            return $this->_marine_forecast;
        }
    ...
    public function WeatherForecast($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_weather_forecast === null) {
                $this->_weather_forecast = new WeatherForecastEntity($this, null);
            }
            return $this->_weather_forecast;
        }
        return new WeatherForecastEntity($this, $data);
    }
