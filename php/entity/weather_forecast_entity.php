...
// OpenMeteo SDK WeatherForecast entity
...
class WeatherForecastEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new WeatherForecastEntity($this->_client, $opts);
    }

    /**
     * @param WeatherForecast|array $args WeatherForecast data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = OpenMeteoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return WeatherForecast|array The current WeatherForecast data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of WeatherForecast fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = OpenMeteoHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of WeatherForecast fields).
     */
    ...
    /**
     * Load a single WeatherForecast.
     *
     * @param WeatherForecastLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed WeatherForecastLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return WeatherForecast|array The loaded WeatherForecast as an assoc-array at the
     *   SDK boundary; throws OpenMeteoError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = OpenMeteoHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
