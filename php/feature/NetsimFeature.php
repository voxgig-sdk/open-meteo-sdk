...
class OpenMeteoNetsimFeature extends OpenMeteoBaseFeature
{
    ...
    private function _simulate(OpenMeteoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(OpenMeteoContext $ctx, array $applied): void
    {
        $client = $this->client;
