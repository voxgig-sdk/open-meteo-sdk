...
class OpenMeteoMakeRequest
{
    public static function call(OpenMeteoContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new OpenMeteoResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new OpenMeteoResponse($fetched);
        } else {
