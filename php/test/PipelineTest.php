...
{
    private static function utility(): OpenMeteoUtility
    {
        return new OpenMeteoUtility();
    }
    ...
    private static function ctx(array $over = []): OpenMeteoContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new OpenMeteoContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new OpenMeteoOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof OpenMeteoError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new OpenMeteoOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = OpenMeteoMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new OpenMeteoOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = OpenMeteoMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new OpenMeteoOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = OpenMeteoMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenMeteoSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = OpenMeteoMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            ...
            'result' => new OpenMeteoResult([]),
        ]);
        [, $err] = OpenMeteoMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new OpenMeteoSpec([]),
            'response' => null,
            'result' => new OpenMeteoResult([]),
        ]);
        [, $err] = OpenMeteoMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new OpenMeteoSpec(['step' => 's']),
            'response' => new OpenMeteoResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new OpenMeteoResult([]),
        ]);
        [, $err] = OpenMeteoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenMeteoSpec(['step' => 's']),
            'response' => new OpenMeteoResponse(self::resp(200, ['v' => 1])),
            'result' => new OpenMeteoResult([]),
        ]);
        [, $err] = OpenMeteoMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new OpenMeteoSpec(['step' => 's']),
            'response' => new OpenMeteoResponse(self::resp(200, ['v' => 2])),
            'result' => new OpenMeteoResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        OpenMeteoMakeResponse::call($ctx);
    ...
    {
        $preset = new OpenMeteoResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new OpenMeteoSpec([]),
            'response' => new OpenMeteoResponse([]),
            'result' => new OpenMeteoResult([]),
        ...
        [$got, $err] = OpenMeteoMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new OpenMeteoResult([])]);
        [, $err] = OpenMeteoMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new OpenMeteoSpec([]), 'result' => null]);
        [, $err] = OpenMeteoMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenMeteoSpec(['step' => 's']),
            'result' => new OpenMeteoResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new OpenMeteoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenMeteoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new OpenMeteoSpec(['step' => 's']),
            'result' => new OpenMeteoResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new OpenMeteoOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = OpenMeteoMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new OpenMeteoResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new OpenMeteoSpec([]),
            'result' => new OpenMeteoResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = OpenMeteoMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenMeteoMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new OpenMeteoError('boom', 'boom');
        $utility->fetcher = function (OpenMeteoContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenMeteoSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = OpenMeteoMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (OpenMeteoContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = OpenMeteoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(OpenMeteoResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (OpenMeteoContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new OpenMeteoSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = OpenMeteoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new OpenMeteoResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new OpenMeteoSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = OpenMeteoMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = OpenMeteoMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = OpenMeteoMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new OpenMeteoResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, OpenMeteoDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new OpenMeteoResult(['ok' => false])]);
        $this->expectException(OpenMeteoError::class);
        OpenMeteoDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new OpenMeteoResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', OpenMeteoMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        OpenMeteoMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new OpenMeteoBaseFeature();
        $b = new OpenMeteoBaseFeature();
        OpenMeteoFeatureAdd::call($ctx, $a);
        OpenMeteoFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): OpenMeteoBaseFeature
    {
        $f = new OpenMeteoBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        OpenMeteoFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): OpenMeteoContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new OpenMeteoSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = OpenMeteoPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new OpenMeteoResponse(['headers' => null]),
            'result' => new OpenMeteoResult([]),
        ]);
        OpenMeteoResultHeaders::call($ctx);
