...
// OpenMeteo SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = OpenMeteoOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(OpenMeteoOmni::structfix($now, Struct::undef()))
                        !== json_encode(OpenMeteoOmni::structfix($val, Struct::undef()))) {
