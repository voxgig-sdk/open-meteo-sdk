...
boru-driven command-line client **and** interactive REPL for the OpenMeteo
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/open-meteo-cli)
make build
...
./open-meteo-cli load 1 historical            # {id:1} shorthand
./open-meteo-cli load '{id:1}' historical       # explicit match map

# 5. Override the API base URL for a single call
OPEN_METEO_BASE=https://api.example.com ./open-meteo-cli load 1 historical

# 6. No arguments -> interactive REPL
./open-meteo-cli
open-meteo> load 1 historical
open-meteo> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/open-meteo-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/open-meteo-cli load 1 historical
   ```
...
```sh
./open-meteo-cli load 1 historical          # scalar shorthand for {id:1}
./open-meteo-cli load '{id:1}' historical     # explicit match map
```
...
./open-meteo-cli load 1 historical
```
...
Run with no arguments to open a REPL (prompt `open-meteo>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `historical`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/open-meteo-cli`. |
...
(`./open-meteo-cli <expr>`) and interactively (the REPL), and expressions compose the
