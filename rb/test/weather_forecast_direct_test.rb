...
class WeatherForecastDirectTest < Minitest::Test
  def test_direct_load_weather_forecast
    setup = weather_forecast_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-weather_forecast", setup[:live] ? "live" : "unit")
    if _should_skip
...
def weather_forecast_direct_setup(mockres)
