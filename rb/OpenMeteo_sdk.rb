...
require_relative 'OpenMeteo_types'
...
class OpenMeteoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = OpenMeteoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = OpenMeteoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, OpenMeteoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    OpenMeteoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = OpenMeteoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = OpenMeteoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = OpenMeteoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = OpenMeteoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = OpenMeteoError.new(
        "graphql_error", "OpenMeteoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
