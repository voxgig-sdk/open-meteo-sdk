# OpenMeteo SDK utility: make_request
...
module OpenMeteoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = OpenMeteoResponse.new({})
    ...
    if fetch_err
      ...
      response = OpenMeteoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = OpenMeteoResponse.new(fetched)
    else
