...
from projectname_sdk import OpenMeteoSDK
from projectname_sdk.core.spec import OpenMeteoSpec
from projectname_sdk.core.result import OpenMeteoResult
from projectname_sdk.core.response import OpenMeteoResponse
from projectname_sdk.core.operation import OpenMeteoOperation
from projectname_sdk.core.error import OpenMeteoError
from projectname_sdk.core import helpers
from projectname_sdk.feature.base_feature import OpenMeteoBaseFeature
...
def _err_from_map(m):
    ...
    if msg == "":
        return None
    code = m.get("code", "")
    return OpenMeteoError(code, msg)
...
class TestPrimaryUtility:
    ...
    def test_feature_init_basic(self):
        init_client = OpenMeteoSDK.test(None, None)
        ...
        class TestInitFeature(OpenMeteoBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "initfeat"
                self.active = True
    ...
    def test_feature_init_inactive(self):
        ...
        class TestInitFeatureInactive(OpenMeteoBaseFeature):
            def __init__(self):
                super().__init__()
                self.name = "nofeat"
                self.active = False
    ...
    def test_make_result_no_result(self):
        ...
        utility = client._utility
        ctx = _make_test_full_ctx(client, utility)
        ctx.spec = OpenMeteoSpec({"step": "start"})
        ctx.result = None
    ...
    def test_make_spec_basic(self):
        ...
        spec_client = OpenMeteoSDK.test(None, setup_opts)
    ...
    def test_prepare_auth_basic(self):
        ...
        auth_client = OpenMeteoSDK.test(None, setup_opts)
