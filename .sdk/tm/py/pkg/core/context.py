...
from projectname_sdk.core.control import OpenMeteoControl
...
class OpenMeteoContext:
    def __init__(self, ctxmap=None, basectx=None):
        ...
        if isinstance(sp, OpenMeteoSpec):
            self.spec = sp
        elif basectx is not None:
        ...
        if isinstance(r, OpenMeteoResult):
            self.result = r
        elif basectx is not None:
        ...
        if isinstance(rp, OpenMeteoResponse):
            self.response = rp
        elif basectx is not None:
    ...
    def make_error(self, code, msg):
        return OpenMeteoError(code, msg, self)
