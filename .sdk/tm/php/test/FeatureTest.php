...
class FtCtrl extends OpenMeteoControl
{
...
class FtClient
{
    ...
    private OpenMeteoUtility $utility;

    public function __construct(OpenMeteoUtility $utility, array $options)
    {
    ...
    public function get_utility(): OpenMeteoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (OpenMeteoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public OpenMeteoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (OpenMeteoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = OpenMeteoConfig::make_feature($name);
    ...
    private static function buildUrl(OpenMeteoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof OpenMeteoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new OpenMeteoResponse($fetched);
        }
    ...
    private function fail(OpenMeteoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (OpenMeteoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
