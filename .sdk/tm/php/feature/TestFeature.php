...
// OpenMeteo SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class OpenMeteoTestFeature extends OpenMeteoBaseFeature
{
    ...
    public function init(OpenMeteoContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (OpenMeteoContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = OpenMeteoParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (OpenMeteoContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(OpenMeteoContext $ctx, $op, $args): array
    {
